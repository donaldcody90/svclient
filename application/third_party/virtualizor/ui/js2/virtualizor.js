...
function done(success){
	...
	if(typeof(success["msg"]) != 'undefined'){
		show_msg((!empty(success['vpsid']) ? "VPSID[" + success['vpsid'] + "] " : "") + success["msg"], "success");
	}
...
function error(er, vpsid){
	...
	if(!empty(vpsid)){
		err_str = "VPSID[" + vpsid + "]" + err_str;
	}
	
	show_msg(err_str, "error");
};
...
function listvs_onload(){
	...
	if(N['user_type'] == 2){
		cols['vps_action'] = {"l" : '{{action}}', "width" : '5%', "centered" : true};
	}
	
	cols["select_all"] = {"l" : '<input type="checkbox" name="select_all" id="select_all" class="select_all" onchange="checkbox_select_all();">', "width" : '1%', "centered" : true};
	...
	for(x in N["vs"]){
		...
		if(N['user_type'] == 2){
			...
			if(empty(N['info']['flags']['disable_cloud_suspend_options'])){
				...
				vpsmanageactbuts += '&nbsp;<a href="#"><img src="[[images]]blank_page.gif" /></a>&nbsp;&nbsp;<a href="#"><img src="[[images]]blank_page.gif" /></a>&nbsp;';
			}
...
function show_hvmsetting_window(){
	...
	$('#tuntap_enable_tr, #ppp_enable_tr , #acpi_tr, #apic_tr, #vnc_tr, #hvm_vnc_keymap_tr, #hvm_vga_tr, #hvm_enable_acceleration_tr, #hvm_sec_iso_tr, #hvm_nic_type_tr, #boot_reorder_pos_tr, #hvm_isos_tr').hide();
			...
			if('isos' in data){
				...
				$('#hvm_isos').html(iso_list);
					if(data['virt'] == 'kvm'){
						...
						$('#hvm_sec_iso_tr').show();
						var sec_iso_list = '<option value="0">{{hvm_none}}</option>';
						for(x in data['isos']){
							
							sec_iso_list += '<option value="'+ x +'" ' + (data['vps']['sec_iso'] == x  ? 'selected="selected"' : '') + '>'+ x +'</option>';
						}
						
						$('#hvm_sec_iso').html(sec_iso_list);
					}
				}
			}
			
			if(data['virt'] != 'openvz' && data['virt'] != 'lxc' && !empty(data['flags']['enable_nic'])){
				var nic_type_list = '';
...
function showvpsmenu(vpsid){
	...
	vpsmenustr += (!empty(N['globals']['novnc']) ? '<a id="novncURL_'+vpsid+'" class="vncButton" href="[[url]]act=vnc&novnc=1&jsnohf=1&svs='+ vpsid +'" target="_blank"><img style="padding-left:4px" src="[[images]]admin/listvs_vnc.gif" title="{{lst_vpmenu_novnc}}"/></a>' : (empty(N['globals']['disable_java_vnc']) ? '<a href="javascript:void(0);" id="java_vnc_'+vpsid+'" class="vncButton" onclick="launchjvnc('+vpsid+')"><img style="padding-left:4px;" src="[[images]]admin/listvs_vnc.gif" title="{{lst_vpmenu_javavnc}}"/></a>' : ''))+'</div></div>';
